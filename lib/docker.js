...
function getTag (container) {
	...
	return axios.get(url)
		.then(({data}) => {
			...
			// is date newer (with the tolerance of 25h)
			const tolerance = 25 * 60 * 60 * 1000;    // 1 day + 1 h
			const isLocalOlder = +data.last_updated > (+container.Image.CreatedAt + tolerance);
