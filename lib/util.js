...
export function findContainerImage (container, images) {
	...
	if (name.startsWith('sha')) tag = '';
	tag = tag || 'latest';
	const shortname = findShortName(name);
	return images.find(i => {
		const itag = (!i.Tag || i.Tag === '<none>') ? 'latest' : i.Tag;
		return (container.Image.includes(i.ID) || i.Repository === name || i.Repository === shortname) && itag === tag;
	});
}
